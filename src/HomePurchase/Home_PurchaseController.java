 ...
 */
public class Home_PurchaseController implements Initializable {
    ...
    public void Save_Print() {
        if (check.isSelected()) {
            ...
            if (data.isEmpty()) {
                dialogs.error_Dialog(main_Pane, "First Add the Items.", "PURCHASE LIST IS EMPTY");
                ...
                try {
                    ...
                    Logger.getLogger(Home_PurchaseController.class.getName()).log(Level.SEVERE, null, ex);
                } finally {
        ...
        if (C5.isEmpty()) {
            ...
            try {
                if (dialogs.confirm_Dialog(main_Pane, "You want to take a print and Save it.")) {
                    ...
                    pst.execute();
                    //finding id
                    rs = logic.getFiledData("purchase", "C1");
                    String Id = "";
                    while (rs.next()) {
                        Id = rs.getString("C1");
                    }
                    ...
                    update.shop_Update("Purchase", C5, Id);
                }
                p_Amount.clear();
    ...
    public void print() {
        try {
            ...
            update.shop_Update("Purchase", C5, Id);
            update.updateStock(Id, "Adding", "purchase_details");
        } catch (SQLException ex) {
            Logger.getLogger(Home_PurchaseController.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            this.CloseConnection();
    ...
    public void Add_Purchase_Details(String Id) {
        try {
            con = SqlConection.ConnectDB();
            for (int i = 0; i < data.size(); i++) {
                ...
                pst.execute();
                //update all_Month
                update.adding_All_Month(data.get(i).getC1(), "purchase");
                update.Update_Products(Id, "purchase", data.get(i).getC1(), temp[0], temp[1], data.get(i).getC2(), data.get(i).getC4());
            }
        } catch (SQLException ex) {
            Logger.getLogger(Home_PurchaseController.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
    ...
    public void Shop_Advance() {
        if (Double.valueOf(advacne_Left) > 0) {
            update.Full_Update(advacne_Left, "purchase", C5);
        }
...
//        }
                ...
                if (newValue != null) {
                    ...
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            p_Previous_Amount.setText(C4);
                            p_Previous_Purchase = C4;
                            p_Cost.setText(C4);
                            p_Total_Cost.setText(C4);
        ...
        p_Scale_List.valueProperty().addListener(new ChangeListener<String>() {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                ...
                if (newValue != null) {
                    ...
                    if (!p_Previous_Purchase.isEmpty()) {
                        String temp = scale.getScale_Amount(newValue, p_Previous_Scale, Double.valueOf(p_Previous_Purchase));
                ...
                if (newValue != null) {
                    ...
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            String previous_Left = rs.getString("C5");
    ...
    private void table_Upload() {
        ...
        for (int i = 0; i < listView.size(); i++) {
            ...
            data.add(new Logic.TableList(String.valueOf(i + 1), temp[0], temp[1], temp[2] + "  " + temp[3], temp[4], remove));
