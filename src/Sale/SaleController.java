...
//      lochal funtion 
    ...
    public void loadData(String m, String y) {
        ...
        try {
            ...
            pst = con.prepareStatement("select * from sale where C3='" + m + "' AND C4='" + y + "'");
            rs = pst.executeQuery();
            int i = 0;
            while (rs.next()) {
    ...
    public void SaveAndPrint() {
        ...
        if (this.C5.isEmpty()) {
            ...
            if (Details.isEmpty()) {
                dialogs.error_Dialog(main_Pane, "First add the item.", "SALE LIST IS EMPTY");
            } else {
                try {
                    con = SqlConection.ConnectDB();
    ...
    public void print() {
        try {
            ...
            all_Updates.shop_Update("Sale", C5, Id);
            all_Updates.updateStock(Id, "Deleting", "sale_details");
        } catch (SQLException ex) {
            Logger.getLogger(SaleController.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            this.CloseConnection();
        ...
        try {
            ...
            for (int i = 0; i < Details.size(); i++) {
                ...
                String sq = "insert into sale_details(C2,C3,C4,C5,C6,C7,C8)values('"
                        ...
                        + "','" + Details.get(i).getC4()
                        + "','" + Details.get(i).getC5() + "')";
                profit = profit + Double.valueOf(Details.get(i).getC5());
                pst = con.prepareStatement(sq);
                pst.execute();
                all_Updates.Update_Products(Id, "sale", Details.get(i).getC1(), temp[0], temp[1], Details.get(i).getC2(), Details.get(i).getC4());
            }
            //Update and Add profit table
            all_Updates.update_Profit(numberFormat.format(profit));
        } catch (SQLException ex) {
            Logger.getLogger(SaleController.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            this.CloseConnection();
    ...
    public void update_Product() {
        ...
        for (int i = 0; i < Details.size(); i++) {
            try {
                String temp[] = Details.get(i).getC3().split("  ");
                rs = logic.getTable_Data("product", Details.get(i).getC1(), "C3", "C1");
                while (rs.next()) {
                    if (!rs.getString("C6").equals(temp[0]) || !rs.getString("C5").equals(temp[1])) {
                        ...
                        String sql = "update product set C4='" + cost
                                + "',C5='" + temp[1]
                                + "',C6='" + temp[0]
                                + "' where C3='" + Details.get(i).getC1() + "'";
                        con = SqlConection.ConnectDB();
                        pst = con.prepareStatement(sql);
                        //Adding operation in stock
                        all_Updates.updateStock(Id, "Adding", "sale_details");
                        //first delete this sale on Id
                        logic.deleteData("sale_details", "C2", Id);
                        //Update
                        pst.executeUpdate();
                        pst.execute();
                        //adding new Sale
    ...
    private void search() {
        searchFiled.setOnKeyReleased(e -> {
            if (searchFiled.getText().equals("")) {
                ...
                data.clear();
                String sql1 = "Select * from sale where C1 like '%" + searchFiled.getText() + "%' And C3='" + m + "' And C4='" + y + "'"
                        + "UNION Select * from sale where C5 like '%" + searchFiled.getText() + "%' And C3='" + m + "' And C4='" + y + "'";
                try {
                    con = SqlConection.ConnectDB();
    ...
    public void Update() {
        ...
        try {
            con = SqlConection.ConnectDB();
            String sql = "update sale set C5='" + C5
                    + "',C6='" + C6
                    + "',C7='" + C7
                    + "',C8='" + C8
                    + "',C9='" + give
            ...
            this.CloseConnection();
            all_Updates.Stock_All_updates("sale");
        }
    }
    ...
    private void update_B_A(ActionEvent event) throws JRException {
        if (sale_Table.getSelectionModel().isEmpty()) {
            ...
            if (listView.isEmpty()) {
                dialogs.error_Dialog(main_Pane, "First add the product.", "SALE LIST IS EMPTY");
            } else {
                this.Update();
            }
        }
        ...
        sale_Table.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                try {
                    ...
                    Id = t.getC1();
                    String check = logic.Check_Id("sale_details", "C2", Id);
                    if (check.isEmpty()) {
                        dialogs.Notofication("Record", "This is your previous record, Cannot update it", "notification");
                        SaleController.this.clear_Filed("");

                    } else {
                        listView.clear();
                        list_Table.getItems().clear();
                        rs = logic.getTable_Data("sale_details", Id, "C2", "C1");
                        while (rs.next()) {
                        ...
                        rs.close();
                        SaleController.this.table_Upload();
                        ...
                        give = logic.getTableValue("sale", "C9", "C1", Id);
                    }
                } catch (SQLException ex) {
                    Logger.getLogger(SaleController.class.getName()).log(Level.SEVERE, null, ex);
                }
    ...
    private void delete_B_A(ActionEvent event) throws JRException {
        if (dialogs.confirm_Dialog(main_Pane, "You want to delete this sale.")) {
            ...
            //Adding operation in stock
            all_Updates.updateStock(Id, "Adding", "sale_details");
            logic.deleteData("sale", "C1", Id);
            logic.deleteData("shop_a", "C5", Id);
            logic.deleteData("sale_details", "C2", Id);
            all_Updates.update_Profit("0.0");
            this.setTable();
            this.loadData(m, y);
            this.clear_Filed("");
            all_Updates.Stock_All_updates("sale");
    ...
    private void table_Upload() {
        ...
        for (int i = 0; i < listView.size(); i++) {
            ...
            Details.add(new Logic.TableList(String.valueOf(i + 1), temp[0], temp[1], temp[2] + "  " + temp[3], temp[4], temp[5], remove));
        }
    ...
    public void change_Scale() {
        scale_List.valueProperty().addListener(new ChangeListener<String>() {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                if (newValue != null) {
                    unit = newValue;
                    if (!previous_Sale.isEmpty()) {
                        String temp = scale.getScale_Amount(newValue, previous_Scale, Double.valueOf(previous_Sale));
    ...
    public void sale_Print(String id) {
